...
describe('5 Journey templates', function () {
  ...
  it('renders the signup journey template', function () {
    cy.get('[data-test-id="journey-template-collection_group"]')
      .find('[data-test-id="journey-template-collection_template"]')
      .each(($e1, index, $list) => {
        const templatetypetext = $e1
          .find('[data-test-id="journey-template-collection_title"]')
          .text();
        if (templatetypetext.includes('Signup')) {
          ...
          cy.get(
            '[data-test-id="journey-template-preview_template-name"]'
    ...
    cy.get('[data-test-id="journey-template-collection_group"]')
      ...
      .each(($e1, index, $list) => {
        ...
        if (templatetypetext.includes('Re-engage: not opened')) {
          ...
          cy.get(
            '[data-test-id="journey-template-preview_template-name"]'
          ).should('have.text', 'Re-engage: not opened');
    ...
    cy.get('[data-test-id="journey-template-collection_group"]')
      ...
      .each(($e1, index, $list) => {
        ...
        if (templatetypetext.includes('Re-engage: not clicked')) {
          ...
          cy.get(
            '[data-test-id="journey-template-preview_template-name"]'
          ).should('have.text', 'Re-engage: not clicked');
          cy.get('[data-test-id="modal_close-button"]').click();
        }
      });
  });
    ...
    cy.get('[data-test-id="journey-template-collection_group"]')
      ...
      .each(($e1, index, $list) => {
        ...
        if (templatetypetext.includes('Custom date field')) {
          ...
          cy.get(
            '[data-test-id="journey-template-preview_template-name"]'
          ).should('have.text', 'Custom date field');
          cy.get('[data-test-id="modal_close-button"]').click();
        }
      });
  });
