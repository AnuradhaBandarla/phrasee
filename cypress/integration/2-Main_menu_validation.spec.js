describe('Main menu validation', function () {
  ...
  it('renders the Analytics page', function () {
    ...
    cy.get('[data-test-id="page-heading_title"]').should('contain.text', 'Analytics');
  });

  it('renders the Integrations page', function () {
    cy.findByText('Integrations').click();
    cy.get('[data-test-id="integrations-signup_heading"]').should('contain.text', 'Connect the dots');
  });
});